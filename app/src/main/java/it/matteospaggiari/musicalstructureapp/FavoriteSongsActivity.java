...
public class FavoriteSongsActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        songs.add(new Song("Save Me", queen, theGame, true));
        songs.add(new Song("Radio Ga Ga", queen, theWorks));
        songs.add(new Song("I Want to Break Free", queen, theWorks, true));
        ...
        songs.add(new Song("Scandal", queen, theMiracle, true));
        ...
        songs.add(new Song("Celebrate", mika, theOriginOfLove,true));
        songs.add(new Song("Symphony No. 9", bach));
        songs.add(new Song("BWV 532 - Prelude and Fugue in D major", bach, true));
        songs.add(new Song("BWV 234 - Mass in A major", bach));
        songs.add(new Song("BWV 525 - Sonata n.1 in Eb major", bach, true));

        // Get the number of elements in the array songs
        int sizeArraySongs = songs.size();

        // Filter the songs to get only the favorite ones
        for(int i=0; i < sizeArraySongs;i++) {
            if(songs.get(i).isFavorite()) {
                favoriteSongs.add(songs.get(i));
            }
        }
